...
public class PlayQuizForm extends VerticalPanel {
	...
	public void sendAnswersLeft() {
		...
		verwaltung.erfassenfehlenderResult(createResultsLeft(), new AsyncCallback<Vector<Boolean>>() {
			@Override
			public void onFailure(Throwable caught) {
				Window.alert(caught.getMessage());
				clicker.setChooseFormToMain();
	...
	public void initButtonsAndImg() {
		...
		if(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer1Text() != null && 
				!questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer1Text().equals("")) {
			answersCount++;
		}
		...
		for (Integer bo : bOrder) {
			switch (bo) {
				case 0:
					buttonFlexTable.setWidget(buttonFlexTable.getRowCount(), 0, createButton(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer1Text()));
					break;
				case 1:
